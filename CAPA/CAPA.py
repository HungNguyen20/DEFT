...
def find_causal_cut(V_set, citobject:CitOracle, oracle_dict , sigma = 3):
            ...
            for index_vj in range(index_vi +1 , len(V_set)):
                if M[index_vi][index_vj] == 0:
                    ...
                    for node_set in all_conditioning_node_sets:
                        key = (V_set[index_vi], V_set[index_vj], tuple(node_set))
                        if key not in oracle_dict:
                            oracle_dict[key] = citobject.query(V_set[index_vi] , V_set[index_vj] , Z = node_set ) #save CIT into oracle_dict (my_dict) for temporary 
                        if oracle_dict[key]:
...
def CAPA(dataset, V_set, groundtruth_struct , options, citobject , oracle_dict):
    ...
    V1 , V2 = find_causal_cut(V_set, citobject=citobject , oracle_dict= oracle_dict , sigma = options['maxCset'])
    if max(len(V1) , len(V2)) == len(V_set):
        ...
        edge_list1 = CAPA(dataset , V1 , groundtruth_struct= groundtruth_struct , options=options, citobject=citobject , oracle_dict = oracle_dict)
        edge_list2 = CAPA(dataset , V2 , groundtruth_struct=groundtruth_struct , options=options , citobject= citobject , oracle_dict= oracle_dict) 
...
if __name__ =="__main__":
    ...
    for i in range(options['repeat']):
        ...
        edge_list=CAPA(dataset=data, V_set=np.arange(d).tolist(),groundtruth_struct = stru_GT, options=options, citobject=citoracle , oracle_dict = oracle_dict)
